...
useEffect(() => {
  const handleScroll = () => {
    ...
    sectionPositions.forEach((sec, i) => {
      if (!sec) return;
      const distance = Math.abs(sec.top);
      if (distance < minDistance && sec.top <= window.innerHeight * 0.4) {
    ...
    if (nearestIndex !== activeIndex) {
      ...
      // Sync animasi highlight
      const liElements = navRef.current?.querySelectorAll("li");
      if (liElements && liElements[nearestIndex]) {
        updateEffectPosition(liElements[nearestIndex]);
